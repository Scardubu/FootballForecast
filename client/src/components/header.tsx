    ...
    async function fetchLeagues() {
      ...
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          // Convert league IDs to strings for consistency with store
        ...
        if (process.env.NODE_ENV === 'development' && error instanceof Error && error.name !== 'AbortError') {
          console.info('Using default leagues:', error.message);
        }
            ...
            <div className="flex items-center space-x-3">
              <div className="md:hidden">
                <MobileMenu links={[
                  ...
                  { href: "/betting-insights", label: "Betting Insights", testId: "mobile-nav-betting" },
                  { href: "/dashboard#predictions", label: "Predictions", testId: "mobile-nav-predictions" },
                  { href: "/dashboard#analytics", label: "Analytics", testId: "mobile-nav-analytics" },
                  { href: "/telemetry", label: "Telemetry", testId: "mobile-nav-telemetry" },
                ]} />
              </div>
